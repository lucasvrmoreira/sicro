...
// 🔹 Link do Menu (Atualizado para aceitar onClick para fechar menu mobile)
function NavLink({ to, children, colorClass, onClick }) {
  ...
  // Cores dinâmicas baseadas na prop colorClass
  const colors = {
...
function AppLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white font-sans">
      ...
      {/* NAVBAR */}
      <nav className="sticky top-0 z-50 bg-gray-900/95 backdrop-blur-xl border-b border-gray-800 shadow-xl px-4 py-3">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between">
          ...
          {/* MENU DESKTOP (Flex horizontal normal) */}
          ...
          <div className="hidden md:block">
            <button
              ...
              className="text-gray-400 hover:text-red-400 text-sm font-medium transition flex items-center gap-1 px-3 py-2 rounded-lg hover:bg-gray-800"
            >
              Sair
        ...
        {/* MENU MOBILE (Dropdown que aparece quando clica no hambúrguer) */}
        {isMenuOpen && (
          <div className="md:hidden absolute top-16 left-0 w-full bg-gray-900 border-b border-gray-800 shadow-2xl animate-slideDown z-50">
            <div className="flex flex-col p-4 space-y-3">
              <NavLink to="/home" colorClass="gray" onClick={() => setIsMenuOpen(false)}>Home</NavLink>
              <NavLink to="/saldo" colorClass="blue" onClick={() => setIsMenuOpen(false)}>Saldo</NavLink>
              <NavLink to="/entrada" colorClass="green" onClick={() => setIsMenuOpen(false)}>Entrada</NavLink>
              <NavLink to="/saida" colorClass="red" onClick={() => setIsMenuOpen(false)}>Saída</NavLink>
              <NavLink to="/historico" colorClass="purple" onClick={() => setIsMenuOpen(false)}>Histórico</NavLink>
              <NavLink to="/planejamento" colorClass="teal" onClick={() => setIsMenuOpen(false)}>Planejamento</NavLink>
              
              <div className="border-t border-gray-800 pt-3 mt-2">
                <button
                  onClick={handleLogout}
                  className="w-full text-center text-red-400 hover:bg-red-900/20 py-3 rounded-lg font-medium transition flex justify-center items-center gap-2"
                >
                  Sair do Sistema
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
      ...
      <main className="p-4 md:p-6 w-full max-w-7xl mx-auto animate-fadeIn pb-12">
        {children}
      </main>
    </div>
  );
